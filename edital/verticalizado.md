# Conteúdo programático verticalizado — Agente de Polícia Federal

> ## ⚠ ORIGEM DESTE ARQUIVO — LEIA ANTES DE CONFIAR
>
> **Não havia PDF de edital nesta pasta quando este arquivo foi gerado
> (23/09/2026), e o acesso de rede a gov.br e ao Cebraspe está bloqueado
> neste ambiente.**
>
> O conteúdo abaixo foi **reconstruído a partir do programa das últimas
> edições do concurso de Agente da Polícia Federal (linha 2021 / 2025)** —
> não foi lido do edital oficial. A estrutura de matérias e o recorte de
> assuntos estão corretos em linha geral, mas **a redação exata dos tópicos,
> a numeração e eventuais inclusões ou exclusões de última hora não estão
> confirmadas na fonte.**
>
> **Substitua este arquivo assim que tiver o PDF.** Jogue o PDF na pasta e
> peça a regeneração — o verticalizado é reescrito e os status já
> conquistados são preservados por nome de assunto.
>
> Enquanto isso, a regra do `CLAUDE.md` continua valendo: **nenhuma questão
> sobre lei, prazo, valor ou artigo é gerada sem confirmação em fonte.** Um
> programa aproximado serve para organizar prioridade; não serve para
> inventar norma.

> **Pesos atualizados em 23/09/2026** para a distribuição em itens da prova
> objetiva de 120 questões, informada pelo Gustavo em auditoria. Ela também
> não pôde ser conferida na fonte. Os assuntos abaixo seguem agrupados por
> matéria do edital; a unidade de estudo da plataforma são os 26 **passos
> estratégicos** de `docs/dados.js`, que reagrupam estes assuntos.

**Status:** `[ ]` não iniciado · `[~]` em andamento · `[x]` consolidado

Regra de promoção (ver `CLAUDE.md` §5): `[~]` a partir da primeira questão
respondida; `[x]` com índice líquido ≥ 0,60 e ao menos 8 questões no
assunto. Índice que cai abaixo de 0,60 devolve o assunto para `[~]`.

---

## Informática — 36 itens

- [ ] Conceitos de hardware: processador, memória, barramentos, dispositivos de E/S
- [ ] Conceitos de software: sistema operacional, aplicativos, licenciamento
- [ ] Sistema operacional Windows: interface, arquivos e pastas, permissões, atalhos
- [ ] Sistema operacional Linux: estrutura de diretórios, permissões, comandos básicos
- [ ] Edição de textos (Word e LibreOffice Writer)
- [ ] Planilhas eletrônicas (Excel e LibreOffice Calc): fórmulas, funções, referências
- [ ] Apresentações (PowerPoint e LibreOffice Impress)
- [ ] Redes de computadores: conceitos, topologias, meios de transmissão
- [ ] Modelo OSI: as sete camadas e suas funções
- [ ] Arquitetura TCP/IP: camadas, correspondência com o OSI
- [ ] Protocolos: IP, TCP, UDP, HTTP, HTTPS, DNS, DHCP, FTP, SMTP, POP3, IMAP
- [ ] Endereçamento: IPv4, IPv6, máscara, sub-redes, NAT
- [ ] Equipamentos de rede: hub, switch, roteador, access point
- [ ] Internet, intranet e extranet
- [ ] Arquitetura cliente-servidor e computação em nuvem (IaaS, PaaS, SaaS)
- [ ] Navegadores: funcionamento, cache, cookies, navegação anônima
- [ ] Correio eletrônico: protocolos, webmail, anexos
- [ ] Segurança da informação: pilares (confidencialidade, integridade, disponibilidade, autenticidade, não repúdio)
- [ ] Malware: vírus, worm, trojan, ransomware, spyware, rootkit, botnet
- [ ] Ataques: phishing, engenharia social, DoS/DDoS, man-in-the-middle, força bruta
- [ ] Criptografia simétrica e assimétrica
- [ ] Assinatura digital, certificado digital, ICP-Brasil, funções de hash
- [ ] Firewall, IDS/IPS, VPN, proxy
- [ ] Autenticação e controle de acesso: senhas, MFA, biometria
- [ ] Backup: tipos (completo, incremental, diferencial), políticas, RPO/RTO
- [ ] Noções de banco de dados e big data

## Língua Portuguesa — 18 itens

- [ ] Compreensão e interpretação de textos
- [ ] Tipologia e gêneros textuais
- [ ] Coesão e coerência textual
- [ ] Ortografia oficial
- [ ] Acentuação gráfica
- [ ] Emprego das classes de palavras
- [ ] Emprego do sinal indicativo de crase
- [ ] Sintaxe da oração e do período
- [ ] Pontuação
- [ ] Concordância nominal e verbal
- [ ] Regência nominal e verbal
- [ ] Colocação pronominal
- [ ] Significação das palavras: sinonímia, antonímia, polissemia
- [ ] Reescrita de frases e substituição de palavras
- [ ] Vozes verbais e transformação de estruturas
- [ ] Redação oficial: características fundamentais (clareza, concisão, formalidade, impessoalidade, padronização)
- [ ] Redação oficial: pronomes de tratamento e concordância
- [ ] Redação oficial: vocativos, endereçamento e fechos
- [ ] Redação oficial: o padrão ofício — estrutura e partes
- [ ] Redação oficial: demais expedientes (exposição de motivos, mensagem, correio eletrônico)
- [ ] Redação oficial: formatação e apresentação dos documentos

## Contabilidade Geral — 20 itens

- [ ] Conceito, objeto, campo de aplicação e finalidade da contabilidade
- [ ] Usuários da informação contábil
- [ ] Princípios e características qualitativas da informação contábil
- [ ] Patrimônio: bens, direitos e obrigações
- [ ] Equação patrimonial e patrimônio líquido
- [ ] Contas: conceito, classificação, teoria das contas
- [ ] Método das partidas dobradas: débito, crédito e saldo
- [ ] Escrituração: livros, lançamentos, fórmulas
- [ ] Fatos contábeis: permutativos, modificativos e mistos
- [ ] Regimes contábeis: caixa e competência
- [ ] Balanço Patrimonial: estrutura, ativo e passivo, critérios de classificação
- [ ] Demonstração do Resultado do Exercício (DRE)
- [ ] Demonstração das Mutações do Patrimônio Líquido (DMPL)
- [ ] Demonstração dos Fluxos de Caixa (DFC): métodos direto e indireto
- [ ] Ativo imobilizado: reconhecimento, depreciação, amortização, exaustão
- [ ] Estoques: critérios de avaliação (PEPS, média ponderada)
- [ ] Contas a receber, provisão para perdas e duplicatas descontadas
- [ ] Provisões, passivos contingentes e ativos contingentes
- [ ] Conciliação bancária
- [ ] Operações com mercadorias e apuração do resultado
- [ ] Análise de balanços: índices de liquidez
- [ ] Análise de balanços: índices de endividamento e estrutura de capital
- [ ] Análise de balanços: índices de rentabilidade
- [ ] Análise horizontal e vertical

## Raciocínio Lógico-Matemático — 12 itens (RLM) + 8 itens (Matemática)

- [ ] Proposições: conceito, valor lógico, proposições simples e compostas
- [ ] Conectivos e tabelas-verdade
- [ ] Tautologia, contradição e contingência
- [ ] Equivalências lógicas (condicional, contrapositiva, De Morgan)
- [ ] Negação de proposições compostas
- [ ] Negação de quantificadores (todo, algum, nenhum)
- [ ] Lógica de argumentação: validade, premissas e conclusão
- [ ] Diagramas lógicos e lógica de primeira ordem
- [ ] Teoria dos conjuntos e diagramas de Venn
- [ ] Análise combinatória: arranjo, combinação, permutação
- [ ] Probabilidade: conceitos, eventos, probabilidade condicional
- [ ] Sequências e séries numéricas
- [ ] Razão, proporção, regra de três, porcentagem
- [ ] Juros simples e compostos
- [ ] Equações, sistemas lineares, matrizes e determinantes
- [ ] Problemas de raciocínio: ordenação, associação, verdades e mentiras
- [ ] Geometria básica: áreas, perímetros, volumes

## Estatística — 4 itens

- [ ] População, amostra e tipos de amostragem
- [ ] Tipos de variáveis: qualitativas e quantitativas
- [ ] Distribuições de frequência, histogramas e representações gráficas
- [ ] Medidas de posição: média, mediana, moda, quartis e percentis
- [ ] Medidas de dispersão: amplitude, variância, desvio padrão, coeficiente de variação
- [ ] Medidas de assimetria e curtose
- [ ] Probabilidade: axiomas, eventos, independência, teorema de Bayes
- [ ] Variáveis aleatórias discretas e contínuas
- [ ] Distribuições: binomial, Poisson, normal
- [ ] Distribuição amostral da média e teorema central do limite
- [ ] Estimação pontual e intervalos de confiança
- [ ] Testes de hipóteses: erros tipo I e II, nível de significância
- [ ] Correlação e regressão linear simples
- [ ] Números-índice

## Direito — 12 itens, repartidos em seis blocos

### Noções de Direito Constitucional
- [ ] Princípios fundamentais
- [ ] Direitos e garantias fundamentais: direitos individuais e coletivos
- [ ] Direitos sociais, nacionalidade e direitos políticos
- [ ] Organização do Estado: União, estados, municípios e competências
- [ ] Administração pública na Constituição (arts. 37 a 41)
- [ ] Organização dos Poderes
- [ ] Segurança pública (art. 144) e atribuições da Polícia Federal

### Noções de Direito Administrativo
- [ ] Administração pública: conceito, princípios expressos e implícitos
- [ ] Organização administrativa: administração direta e indireta
- [ ] Poderes administrativos e abuso de poder
- [ ] Atos administrativos: conceito, requisitos, atributos, espécies, extinção
- [ ] Agentes públicos e regime jurídico (Lei 8.112/1990)
- [ ] Processo administrativo disciplinar
- [ ] Improbidade administrativa (Lei 8.429/1992)
- [ ] Licitações e contratos (Lei 14.133/2021)
- [ ] Controle da administração e responsabilidade civil do Estado
- [ ] Processo administrativo federal (Lei 9.784/1999)

### Noções de Direito Penal
- [ ] Aplicação da lei penal no tempo e no espaço
- [ ] Teoria do crime: fato típico, ilicitude, culpabilidade
- [ ] Excludentes de ilicitude e de culpabilidade
- [ ] Concurso de pessoas e concurso de crimes
- [ ] Penas: espécies, aplicação e dosimetria
- [ ] Extinção da punibilidade e prescrição
- [ ] Crimes contra a pessoa e contra o patrimônio
- [ ] Crimes contra a fé pública
- [ ] Crimes contra a administração pública

### Noções de Direito Processual Penal
- [ ] Inquérito policial: natureza, características, atribuições
- [ ] Ação penal pública e privada
- [ ] Prova: meios, ônus, provas ilícitas, cadeia de custódia
- [ ] Prisão em flagrante, preventiva e temporária
- [ ] Medidas cautelares diversas da prisão
- [ ] Audiência de custódia
- [ ] Competência e jurisdição
- [ ] Habeas corpus e demais ações de impugnação

### Legislação Especial
- [ ] Lei 10.826/2003 — Estatuto do Desarmamento
- [ ] Lei 11.343/2006 — Lei de Drogas
- [ ] Lei 9.455/1997 — Crimes de tortura
- [ ] Lei 13.869/2019 — Abuso de autoridade
- [ ] Lei 12.850/2013 — Organização criminosa
- [ ] Lei 9.613/1998 — Lavagem de dinheiro
- [ ] Lei 12.830/2013 — Investigação criminal conduzida por delegado
- [ ] Lei 8.069/1990 — ECA (disposições penais)
- [ ] Lei 13.445/2017 — Lei de Migração
- [ ] Lei 7.716/1989 — Crimes de preconceito e racismo

## Matemática — 8 itens

- [ ] Razão, proporção, regra de três, porcentagem
- [ ] Juros simples e compostos
- [ ] Análise combinatória: arranjo, combinação, permutação
- [ ] Probabilidade: conceitos, eventos, probabilidade condicional

> Estes assuntos também constam do bloco de Raciocínio Lógico-Matemático
> acima. Na contagem de itens da prova eles são medidos à parte, como
> Matemática, e é assim que a plataforma os pesa.

## Contabilidade Pública — 4 itens

- [ ] Orçamento público: princípios e ciclo orçamentário (PPA, LDO, LOA)
- [ ] Receita pública: classificação e estágios
- [ ] Despesa pública: classificação e estágios
- [ ] Restos a pagar e regime contábil público
- [ ] Exercício financeiro e Lei 4.320/1964

## Direito Ambiental — 1 item

- [ ] Direito Ambiental: princípios e competências
- [ ] Direito Ambiental: crimes ambientais e responsabilidade
